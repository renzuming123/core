...
class LockMapper extends Mapper {
	...
	public function getLocksByPath($storageId, $internalPath, $returnChildLocks) {
		...
		$query->select(['id', 'owner', 'timeout', 'created_at', 'token', 'scope', 'depth', 'file_id', 'path', 'account_id'])
	...
	public function getLockByToken($token) {
		...
		$query->select(['id', 'owner', 'timeout', 'created_at', 'token', 'scope', 'depth', 'file_id', 'account_id'])
			->from($this->getTableName(), 'l')
